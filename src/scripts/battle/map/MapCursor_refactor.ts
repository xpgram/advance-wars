...
export class MapCursor extends Observable {
    ...
    set mode(mode) {
        ...
        this.pointerSprite.visible = (mode !== 'target');
        this.cursorSprite.textures = cursor;
        this.cursorSprite.anchor.set(
            (mode === 'target') ? 0.25 : 0.136364,  // TODO How do I extract these from the texture json?
            (mode === 'target') ? 0.25 : 0.136364,
        )
        this._mode = mode;
    }
