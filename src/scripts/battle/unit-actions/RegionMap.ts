...
export class RegionMap {
  ...
  /** Returns true if the given point is located on the map described, or false if it
   * is only referenceable via the infinite plane projection. */
  onMap(point: Point) {
    let m = point.add(this.origin);
    return Common.validIndex(m.x, this.width) && Common.validIndex(m.y, this.height);
  }
  ...
  get(point: Point) {
    let m = point.add(this.origin);
    return (this.onMap(point)) ? this._map[m.x][m.y] : false;
  }
  ...
  /** Returns a new RegionMap object equivalent to this one after performing a
   * bucket-fill action starting with the given point. */
  fill(point: Point): RegionMap {
    const map = this.mapCopy();

    const fill = (p: Point) => {
      let m = p.add(this.origin);
      if (!this.onMap(p) || map[m.x][m.y])
        return;
      map[m.x][m.y] = true;
      for (const vector of Point.Cardinals)
        fill(p.add(vector))
    }
